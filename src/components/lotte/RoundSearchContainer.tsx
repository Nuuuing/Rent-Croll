// src/components/lotte/RoundSearchContainer.tsx
'use client';
    ...
    return (
        <div className="min-h-screen w-full flex flex-col md:flex-row gap-8 p-8 items-stretch">
            ...
            <div className="md:w-3/5 w-full flex flex-col gap-8 h-full">
                <div className="rounded-2xl bg-[#282a45] shadow-xl p-8 border border-[#35365a] min-h-[800px] flex flex-col h-full">
                    ...
                    {!fetchingCars && processedAllCars.length > 0 ? (
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-4 max-h-[660px] overflow-y-auto pr-2 scrollbar-thin scrollbar-thumb-[#44446a] scrollbar-track-[#282a45]">
                            {processedAllCars.map((car, idx) => (
                                <div key={`${car.areaCode}-${car.placeCode}-${car.code || idx}`} className="rounded-xl bg-[#31335a] p-6 shadow-lg border border-[#3a3b5c] flex flex-col gap-2 hover:scale-[1.02] transition-transform duration-200">
                                    ...
                                    <div className="text-gray-400 text-xs mb-2">지점: <span className="text-blue-300">{car.areaName} ({car.areaCode})</span></div>
                    ...
                    ) : !fetchingCars && !searchError && processedAllCars.length === 0 && rawAllAreaCars !== null && (
                        <div className="flex flex-col items-center justify-center flex-1 text-gray-400 text-lg">
                            입력된 조건에 맞는 차량이 없습니다.
                        </div>
                    )}
                    {!fetchingCars && processedAllCars.length === 0 && rawAllAreaCars === null && !searchError && (
                        <div className="flex flex-col items-center justify-center flex-1 text-gray-400 text-lg">
                            날짜와 시간을 입력하고 '차량 검색' 버튼을 눌러주세요.
                        </div>
                    )}
                </div>
            </div>
