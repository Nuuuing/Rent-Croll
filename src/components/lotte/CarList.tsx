...
import { AreaCarData, FlattenedCarData } from "@/types/lotte";
import { getFinalDisplayPrice } from "@/utils/car";

interface CarListProps {
    fetchingState: boolean;
    searchState: string | null;
    processedCars: FlattenedCarData[];
    rawAllAreaCars: AreaCarData[] | null;
}

export const CarList = (props: CarListProps) => {
    const { fetchingState, searchState, processedCars, rawAllAreaCars } = props;
            ...
            <div className="rounded-2xl bg-[#282a45] shadow-xl p-8 border border-[#35365a] flex flex-col h-full">
                ...
                {fetchingState && (
                    <div className="flex flex-col items-center justify-center flex-1">
                        ...
                        <div className="text-lg text-blue-300 font-semibold">차량 정보를 불러오는 중... %</div>
                    </div>
                )}
                {searchState && (
                ...
                {!fetchingState && processedCars.length > 0 ? (
                    ...
                    <div className="flex flex-col items-center justify-center flex-1 text-gray-400 text-lg">
                        <span>날짜, 시간, 픽업/반납 지점을 입력하고 &#39;차량 검색&#39; 버튼을 눌러주세요.</span>
                    </div>
                )}
                {!fetchingState && rawAllAreaCars !== null && (
                    <div className="flex flex-col items-center justify-center flex-1 text-gray-400 text-lg">
                        입력된 조건에 맞는 차량이 없습니다.
                    </div>
                )}
